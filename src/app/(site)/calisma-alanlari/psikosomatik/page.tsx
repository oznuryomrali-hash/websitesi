import type { Metadata } from 'next'
import Link from 'next/link'
import ContactSection from '@/components/site/ContactSection'

export const metadata: Metadata = {
  title: 'Psikosomatik ve Duygusal Zorluklar',
  description:
    'Nedeni açıklanamayan bedensel ağrılar, kronik ağrılar, kaygı, panik atak, depresyon, yas ve kronik stres üzerine psikoterapi.',
  alternates: { canonical: '/calisma-alanlari/psikosomatik' },
}

const topics = [
  'Nedeni açıklanamayan bedensel ağrılar, belirtiler ve yakınmalar',
  'Kronik ağrılar',
  'Kaygı',
  'Panik atak',
  'Depresyon',
  'Yas süreçleri',
  'Kronik stres',
]

export default function PsikosomatikPage() {
  return (
    <>
      <div className="pt-32 pb-16 md:pt-40 md:pb-20 bg-warm-sand/50">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-gutter">
          <Link
            href="/calisma-alanlari"
            className="inline-flex items-center gap-2 font-label text-label-md text-on-surface-variant hover:text-primary transition-colors mb-8"
          >
            <span className="material-symbols-outlined text-base">arrow_back</span>
            Çalışma Alanlarım
          </Link>
          <span className="font-label text-label-md text-ocean-muted uppercase tracking-widest block">
            Çalışma Alanı
          </span>
          <h1 className="font-headline text-headline-lg md:text-display-lg text-primary mt-4 mb-6">
            Psikosomatik ve Duygusal Zorluklar
          </h1>
          <p className="font-body text-body-lg text-on-surface-variant max-w-2xl">
            Beden ve zihin birbirinden ayrı değildir. Zaman zaman içimizde taşıdığımız zorluklar
            beden üzerinden dışa vurur: ağrılar, belirtiler, tükenmişlik. Bu alanlarda birlikte
            çalışabiliriz.
          </p>
        </div>
      </div>

      <section className="py-section-gap-mobile md:py-section-gap-desktop">
        <div className="max-w-container-max mx-auto px-margin-mobile md:px-gutter">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            <div className="bg-surface rounded-xl p-8 border border-warm-sand soft-card-shadow">
              <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center text-primary mb-6">
                <span className="material-symbols-outlined">monitor_heart</span>
              </div>
              <h2 className="font-headline text-headline-md text-primary mb-6">Çalışılan Konular</h2>
              <ul className="space-y-4">
                {topics.map((topic) => (
                  <li key={topic} className="flex items-start gap-3 font-body text-body-md text-on-surface-variant">
                    <span className="material-symbols-outlined text-ocean-muted text-base mt-0.5 flex-shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>
                      check_circle
                    </span>
                    {topic}
                  </li>
                ))}
              </ul>
            </div>

            <div className="space-y-6">
              <div className="bg-soft-mist rounded-xl p-8 border border-outline-variant">
                <h2 className="font-headline text-headline-md text-primary mb-4">Nasıl Çalışıyorum?</h2>
                <p className="font-body text-body-md text-on-surface-variant">
                  Psikosomatik yakınmalarda beden ve zihin arasındaki köprüyü anlamak önemlidir.
                  Tıbbi açıklaması olmayan ağrılar veya belirtiler çoğu zaman duygusal içerikler
                  taşır. Bu içerikleri birlikte keşfetmek hem belirtileri hem de altındaki anlamı
                  dönüştürebilir.
                </p>
              </div>
              <div className="bg-ocean-muted/10 border border-ocean-muted/20 rounded-xl p-6">
                <p className="font-body text-body-md text-on-surface-variant">
                  Kaygı, depresyon veya panik atak gibi durumlarda psikoterapi ile ilaç
                  tedavisinin birlikte yürütülmesi önerilebilir. Bu konularda psikiyatrist ile
                  koordineli çalışılabilir.
                </p>
              </div>
              <div className="flex flex-wrap gap-4">
                <a
                  href="https://wa.me/905343500675?text=Merhaba%2C%20bilgi%20almak%20istiyorum."
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-primary text-on-primary px-8 py-3 rounded-full font-label text-label-md hover:opacity-80 transition-opacity"
                >
                  Seans al
                  <span className="material-symbols-outlined text-base">arrow_forward</span>
                </a>
                <Link
                  href="/calisma-alanlari"
                  className="inline-flex items-center gap-2 border border-primary text-primary px-8 py-3 rounded-full font-label text-label-md hover:bg-primary hover:text-on-primary transition-all"
                >
                  Diğer konular
                </Link>
              </div>
            </div>
          </div>
        </div>
      </section>

      <ContactSection />
    </>
  )
}
